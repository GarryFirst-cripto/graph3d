...
export default Kapsule({
  ...
  props: {
    ...
    onNavigating: { default: () => { }, triggerUpdate: false },
    onNavigatingEnd: { default: () => { }, triggerUpdate: false },
  ...
  init: function(domNode, state) {
    ...
    roDomNode.addEventListener('mousedown', function (event) {
      if (event.which === 1 && Boolean(!state.hoverObj)) {
        state.isNavigating = true;
      };
    })
    roDomNode.addEventListener('mousemove', function (event) {
      if (state.isNavigating) {
        controls.enabled = state.onNavigating(event)
      }
    })
    roDomNode.addEventListener('mouseup', function (event) {
      if (state.isNavigating) {
        state.onNavigatingEnd(event);
        state.isNavigating = false;
      }
    })
